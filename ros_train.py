...
def main(config: ROSTrainerConfig) -> None:
    ...
    if config.max_num_iterations:
        CONSOLE.log(
            "Using --max-num-iterations alias for --pipeline.datamanager.max_num_iterations"
        )
    ...
    if config.pipeline.datamanager.camera_optimizer.mode == "off":
        ...
        config.pipeline.datamanager.use_new_dataloader = False
    elif config.pipeline.datamanager.camera_optimizer.mode:
        CONSOLE.log("Using New DataLoader")
        config.pipeline.datamanager.use_new_dataloader = True

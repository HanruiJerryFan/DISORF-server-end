...
from nsros.ros_pipeline import ROSPipeline
from nsros.ros_utils import relocate_camera
...
class ROSTrainer(Trainer):
    ...
    def setup(self, test_mode: Literal["test", "val", "inference"] = "val"):
        ...
        if self.config.disable_streaming:
            CONSOLE.print(
                f"[bold green] (NSROS) Streaming is disabled. No more images will be loaded ...."
            )
            ...
            # transmission auto start, some dirty codes here.
            if self.config.auto_transmission:
                scene_name = self.config.data.parts[-1]
                # Replica
                # data_dir = '../nerfbridge_experiment/data/replica_slam/undistorted'
                # frame_rate = 30
                # TNT
                # data_dir = '../nerfbridge_experiment/data/tank_and_temples_colmap/exp_720p'
                # frame_rate = 1
                # TNT+SLAM
                # data_dir = '../nerfbridge_experiment/data/tank_and_temples_slam'
                # scene_name = 'pcgpu_' + scene_name.lower()
                # frame_rate = 30
                # TNT+Droid
                data_dir = '../nerfbridge_experiment/data/tank_and_temples_droid'
                frame_rate = 23
                cmd = f"python ../nerfbridge_experiment/ros_posed_img_transfer.py " + \
        ...
        if self.config.relocate_cameras:
            ...
            CONSOLE.print(
                f"[bold green] (NSROS) Relocating cameras by {translation} and scaling by {scale} ...."
            )
            if self.config.disable_streaming:
                self.dataset.cameras.camera_to_worlds[:, :3, 3] = self.dataset.cameras.camera_to_worlds[:, :3, 3] * scale + translation
    ...
    def _update_viewer_state(self, step: int, update_all=False):
        ...
        if self.config.draw_training_images:
            ...
            for idx in image_indices:
                if not idx in self.cameras_drawn or update_all:
        ...
        with TimeWriter(writer, EventName.TOTAL_TRAIN_TIME):
            ...
            for step in range(self._start_step, self._start_step + num_iterations):
                ...
                if self.pipeline.datamanager.train_image_dataloader.finished:
                    self.fisished_iter += 1
                    if self.fisished_iter > 100:
                        break

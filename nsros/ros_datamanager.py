...
from typing import Type, Dict, Tuple, Literal, List
import torch
...
from nsros.ros_dataloader import ROSDataloader, ROSFixedIndicesEvalDataloader, ROSRandIndicesEvalDataloader
...
class ROSDataManagerConfig(base_datamanager.VanillaDataManagerConfig):
    ...
    training_sampler: Literal[
        ...
        "alternating",
        "freq", 
        "inversely_propotional", 
        "inverse_delta", "inverse_delta_fix",
        "loss_weighted_frame", "loss_weighted_patch",
        "imap",
        "hierarchical_loss"
        ] = "default"
    """ Sampler to use for training. """
    multi_sampler_list: List[str] = field(default_factory=lambda: [])
    """ List of samplers to use for training, when blending or alternating samplers are used. """
    multi_sampler_ratio: List[float] = field(default_factory=lambda: [])
    """ Blending ratio for the blending sampler. """
    max_num_iterations: int = 8000
    """ Maximum number of iterations to train for. """
    use_new_dataloader: bool = True
...
class ROSDataManager(
    ...
    def setup_train(self):
        ...
        self.train_image_dataloader = ROSDataloader(
            ...
            self.config.use_new_dataloader,
            device=self.device,
            num_workers=0, # maybe ...
            pin_memory=True,
        ...
        if self.config.training_sampler in ["blending", "alternating", "imap", "hierarchical_loss"]:
            training_samplers += self.config.multi_sampler_list
        for sampler in training_samplers:
            if sampler in ["freq", "inversely_propotional"]:
                self.config.track_sample_cnt = True
            if "loss" in sampler:
                self.config.track_perpixel_loss = True
            if sampler in ["imap", "inverse_delta", "inverse_delta_fix"]:
            ...
            if sampler == 'blending':
                ...
                multi_sampler_ratio = np.array(self.config.multi_sampler_ratio)
                multi_sampler_ratio /= np.sum(multi_sampler_ratio)
                self.rays_per_sampler = np.floor(total_rays_per_batch * multi_sampler_ratio).astype(np.int32)
        ...
        if self.config.track_perpixel_loss:
            self.pixel_loss = torch.zeros(self.train_image_dataloader.dataset.image_tensor.shape[:3], device=self.device)
            self.pixel_sample_cnt = torch.zeros(self.train_image_dataloader.dataset.image_tensor.shape[:3], dtype=torch.int32, device=self.device)
            # patch level
            ...
            patch_loss = torch.nn.functional.avg_pool2d(self.pixel_loss, self.patch, ceil_mode=False, divisor_override=1) # [Bs, Ps, Ps]
            patch_valid = torch.nn.functional.avg_pool2d((self.pixel_sample_cnt > 0).float(), self.patch, ceil_mode=False, divisor_override=1) # [Bs, Ps, Ps]
    ...
    def _default_sample(self, available_frames: int, num_samples: int, eariliest_frame: int = 0, img_indices_only: bool = False):
        if img_indices_only:
            image_indices = torch.floor(torch.rand((num_samples, 1), device=self.device)
                * (available_frames - eariliest_frame)).long() + eariliest_frame
    ...
    def _freq_sample(self, available_frames: int, num_samples: int, eariliest_frame: int = 0, img_indices_only: bool = False):
        total_iteration = self.config.max_num_iterations # this is unknown!
        expected_sample_times_per_image = \
            self.config.train_num_rays_per_batch * total_iteration / self.config.num_training_images
        sampling_required = expected_sample_times_per_image - \
            self.frame_sample_cnt[eariliest_frame:available_frames]
        sampling_required = torch.abs(sampling_required)
        sampling_required = torch.max(sampling_required) - sampling_required + 1e-5
        image_indices, pix_indices = multinomial_ray_sample(
                sampling_required, num_samples, self.height, self.width, self.device, img_indices_only
        )
        if img_indices_only:
            return image_indices
    ...
    def _inversely_propotional_sample(self, available_frames: int, num_samples: int, eariliest_frame: int = 0, img_indices_only: bool = False):
        sampling_required = torch.max(self.frame_sample_cnt) - \
            self.frame_sample_cnt[eariliest_frame:available_frames] * 0.95 + 1e-5
        image_indices, pix_indices = multinomial_ray_sample(
                sampling_required, num_samples, self.height, self.width, self.device, img_indices_only
        )
        if img_indices_only:
    ...
    def _loss_weighted_patch_sample(self, available_frames: int, num_samples: int, eariliest_frame: int = 0, img_indices_only: bool = False):
        base_loss = 0.001
    ...
    def _loss_weighted_frame_sample(self, available_frames: int, num_samples: int, eariliest_frame: int = 0, img_indices_only: bool = False):
        base_loss = 0.001
        frame_loss = self.frame_loss[eariliest_frame:available_frames]
        frame_loss = frame_loss + torch.mean(frame_loss) * 0.5 + base_loss
        image_indices, pix_indices = multinomial_ray_sample(
            frame_loss, num_samples, self.height, self.width, self.device
        )
        if img_indices_only:
            return image_indices
        indices = torch.cat((image_indices.unsqueeze(-1) + eariliest_frame, pix_indices), dim=1)
    ...
    def _blending_sample(self, available_frames: int, num_samples: int, eariliest_frame: int = 0, img_indices_only: bool = False):
        multi_indices = []
    ...
    def _alternating_sample(self, available_frames: int, num_samples: int, eariliest_frame: int = 0, img_indices_only: bool = False):
    ...
    def _imap_sample(self, available_frames: int, num_samples: int, eariliest_frame: int = 0, img_indices_only: bool = False):
        ...
        if (self.train_count - self.last_timestamp) <= 0.5 * 1 / self.avg_frame_speed:
            ...
            latest_indices = getattr(self, f"_{inner_sampler}_sample")(available_frames, latest_ray_num, available_frames-1)
    ...
    def _inverse_delta_sample(self, available_frames: int, num_samples: int, eariliest_frame: int = 0, img_indices_only: bool = False):
        ...
        # scale, ratio = 1, 9
        sample_weight = torch.exp(-(time_delta - 1) * max(self.avg_frame_speed * scale, 0.01)) + ratio/available_frames
        image_indices, pix_indices = multinomial_ray_sample(
    ...
    def _inverse_delta_fix_sample(self, available_frames: int, num_samples: int, eariliest_frame: int = 0, img_indices_only: bool = False):
        time_delta = self.train_count - 1 - self.frame_timestamp[eariliest_frame:available_frames] + 1e-3
        sample_weight = torch.exp(-(time_delta - 1) * max(self.avg_frame_speed * 0.5, 0.01)) + 0.8
        image_indices, pix_indices = multinomial_ray_sample(
            sample_weight, num_samples, self.height, self.width, self.device, img_indices_only
        )
        if img_indices_only:
    ...
    def _hierarchical_loss_sample(self, available_frames: int, num_samples: int, eariliest_frame: int = 0, img_indices_only: bool = False):
        frame_sampler = self.config.multi_sampler_list[0]
        # recent frames
        recent_iters = 10
        recent_frames = ((self.frame_timestamp[:available_frames] + recent_iters \
                          - self.train_count + 1) >= 0).sum().item()
        time_delta = self.train_count - 1 - self.frame_timestamp[eariliest_frame:available_frames] + 1e-3
        ...
        if recent_num_samples > 0:
            ...
            indices = self._loss_weighted_patch_sample(available_frames, recent_num_samples, available_frames - recent_frames)
    ...
    def next_train(self, step: int) -> Tuple[RayBundle, Dict]:
        ...
        if self.replay_trans_log:
            received_image_number = self.avail_images_per_iter[self.train_count] \
                ...
                else self.avail_images_per_iter[-1]
            _batch = {}
    ...
    def setup_eval(self):
        ...
        if self.config.eval_with_training_set:
            camera_optimizer = self.train_camera_optimizer
        else:
            camera_optimizer = self.config.camera_optimizer.setup(
    ...
    def get_training_callbacks(
        ...
        def track_sample_cnt_status(self, step: int):
            ...
            self.frame_sample_cnt.index_add_(0, 
                        image_indices, 
                        torch.ones_like(image_indices, dtype=self.frame_sample_cnt.dtype)
                    )
            # if step >= 1500:
        ...
        def track_perpixel_loss_status(self, step: int):
            ...
            avail_pixel_loss *= 0.95
            curr_batch_loss = self.pixel_loss[pix_indices[:, 0], pix_indices[:, 1], pix_indices[:, 2]]
            self.pixel_loss[pix_indices[:, 0], pix_indices[:, 1], pix_indices[:, 2]] = torch.maximum(curr_batch_loss, batch_metric)
            self.pixel_sample_cnt[pix_indices[:, 0], pix_indices[:, 1], pix_indices[:, 2]] += 1
            avg_patch_loss = torch.nn.functional.avg_pool2d(avail_pixel_loss, self.patch, ceil_mode=False,  divisor_override=1) # [Bs, Ps, Ps]
            avg_patch_cnt  = torch.nn.functional.avg_pool2d((avail_patch_cnt > 0).float(), self.patch, ceil_mode=False,  divisor_override=1) # [Bs, Ps, Ps]
            avail_patch_loss[:] = avg_patch_loss / (avg_patch_cnt + 1e-3) # [Bs, Ps, Ps]
            avail_frame_loss[:] = avail_patch_loss.sum(dim=(1,2)) / ((avail_patch_loss != 0).sum(dim=(1,2)) + 1e-3) # [Bs]
             # [Bs]
...
def multinomial_ray_sample(sample_weight, number_samples, h, w, device, image_indices_only=False):
    ...
    pix_indices = torch.floor(
        torch.rand((number_samples, 2), device=device)
...
def plot_samples_debug(image_indices, train_count, output_dir):
    ...
    plt.xlim(0, 150); plt.ylim(0, 4096)
    plt.title(f'Sampling Distribution | Iteration {train_count - 1}')
    save_path = output_dir / f"sample_distr"

...
class ROSDataloader(DataLoader):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.use_new_dataloader = use_new_dataloader
    ...
    def ts_image_pose_callback(self, image: Image, pose: PoseStamped):
        ...
        if (
                now - self.last_update_t > self.update_period and \
                self.current_idx < self.num_images
        ):
            ...
            if self.current_idx not in self.dataset.updated_indices:
                ...
                if self.use_new_dataloader:
                    if self.camera_optimizer is not None and self.camera_optimizer.config.mode != 'off':
...
class ROSFixedIndicesEvalDataloader(FixedIndicesEvalDataloader):
    ...
    def __next__(self):
        if self.count < len(self.image_indices):
            ...
            with torch.no_grad():
                camera_opt_to_camera = self.camera_optimizer(torch.tensor([self.count])) \
                    if self.camera_optimizer is not None else None
            ray_bundle, batch = _get_data_from_image_idx(self, image_idx, camera_opt_to_camera)
            self.count += 1
            return ray_bundle, batch
        raise StopIteration
...
class ROSRandIndicesEvalDataloader(RandIndicesEvalDataloader):
    ...
    def __next__(self):
        ...
        with torch.no_grad():
            camera_opt_to_camera = self.camera_optimizer([image_idx]) \
                if self.camera_optimizer is not None else None
        ray_bundle, batch = _get_data_from_image_idx(self, image_idx, camera_opt_to_camera)
        return ray_bundle, batch